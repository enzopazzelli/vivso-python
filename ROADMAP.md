...
| 2026-06-16 | **Criterios visibles en el dashboard (para la reunión con el área)** | Nuevo componente `dashboard/components/criterios.py`: un solo lugar con el criterio de cada indicador y su etiqueta `[S#]`. Cada sección del dashboard (KPIs, mapa, riesgo, cuello de botella, cobertura, tiempos…) tiene ahora un desplegable *"ℹ️ Cómo se calcula / criterio"* que explica el cálculo y apunta a `docs/datos-a-confirmar.md`. Objetivo: que el responsable del área vea, indicador por indicador, qué supuesto se puede corregir. Verificado con `streamlit.testing` (7 páginas OK). |
