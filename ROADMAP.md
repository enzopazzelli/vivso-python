...
- [ ] Presentación Hito 3 + guion (lecciones de la devolución del Hito 2): As is → To be con números de ganancia, tablas de ejemplo, código casi cero; guion como mapa de defensa con protocolo de preparación — nadie memoriza, nadie lee.
...
| 2026-06-16 | **Notas "cómo se calcula" en el dashboard (para la reunión con el área)** | Nuevo componente `dashboard/components/criterios.py`: una nota **breve** por sección con cómo se calcula el indicador y **de qué tabla** salen los datos (vivienda, visita, avance_rubro, organizacion…). Cada sección del dashboard tiene un desplegable *"ℹ️ Cómo se calcula"* con esa nota y un puntero a `docs/datos-a-confirmar.md`. Objetivo: que el responsable del área entienda de un vistazo qué está viendo y de dónde sale, sin un glosario de supuestos. Verificado con `streamlit.testing` (7 páginas OK). |
| 2026-06-16 | **Limpieza de docs para compartir el repo con Desarrollo** | Eliminados los docs internos de presentación (`guia-presentaciones.md`, `guion-presentacion-hito2.md`, `prompt-pptx-hito2.md`) — son material de cátedra, no aportan al equipo de Programación. Creado `docs/para-desarrollo.md`: punto de entrada para Desarrollo (qué es el componente, integración `cd_*`, brechas, pedidos, modelo de riesgo) que consolida lo que estaba disperso en este ROADMAP. Referencias actualizadas en README y ROADMAP. |
...
- Guía de integración para Desarrollo: `docs/para-desarrollo.md`
