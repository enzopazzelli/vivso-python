...
- [ ] **Informe EDA** (la tarea principal): consolidar en un único documento formal lo repartido entre los colabs y `docs/documentacion-analisis.md` — dataset y preprocesamiento justificado, hallazgos con figuras, indicadores, modelo de riesgo. Encuadrar el proyecto en los tipos de solución de la cátedra: **Panel** (match literal: "tablero de indicadores operativos para dirección pública") + **Pipeline** (ETL) + **CV** (OCR como prototipo).
- [ ] **Prototipo pulido para demo**: el dashboard + ETL + BDD local ya funcionan; regenerar procesados, verificar coherencia de datos y ensayar el flujo completo.
...
| | | *(próxima entrada: acuerdos de la reunión WS0 con Desarrollo)* |
